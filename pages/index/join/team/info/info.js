    ...
    if (that.data.tempFilePaths[i].indexOf(servsers) >= 0) {
      ...
      if (i == that.data.tempFilePaths.length) {
        ...
        wx.request({
          ...
          success: function (res) {
            var ch_status = 0;
            for (var i = 0; i < app.globalData.team_enroll.length; i++) {
              if (app.globalData.team_enroll[i].id_number == res.data.id_number) {
                app.globalData.team_enroll.splice(i, 1);
                app.globalData.team_enroll.push(res.data);
                ch_status = 1;
                break;
              }
            }
